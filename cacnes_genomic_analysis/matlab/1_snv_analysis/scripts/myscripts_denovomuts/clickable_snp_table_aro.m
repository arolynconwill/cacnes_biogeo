function clickable_snp_table_aro(annotations, callsi, cnts, SampleNames, ScafNames, MutQual, MutQual_fracambig, QualSort, coverageMatrix, p)

%% Changes
% % Arolyn, 2021.02: Adding some extra columns
...
if numel(ScafNames)>1
    colnames={'Qual', 'QualAlt', 'Type','Scaf','Pos', 'Locustag', 'Gene','Annotation', 'AApos', 'NTs', 'AAs'};
    widths=num2cell([48, 48, 16, 12, 58, 50, 38, 250, 42, 40, 40, ones(1,numel(SampleNames))*26]);
else
    colnames={'Qual', 'QualAlt', 'Type','Pos', 'Locustag','Gene','Annotation', 'AApos', 'NTs', 'AAs'};
    widths=num2cell([48, 48, 16, 58, 50, 38, 250, 45, 40, 40, ones(1,numel(SampleNames))*26]);
end
...
for i=1:numel(annotations)
    ...
    if numel(ScafNames)>1
        tabledata(i,1:nonsamplecols)=[{MutQual(i)} {MutQual_fracambig(i)} {annotations(i).type} {annotations(i).scaffold} {annotations(i).pos} ...
            {locustag} {annotations(i).gene} {annotations(i).annotation} {annotations(i).AApos} {[annotations(i).nts]} ...
            {[annotations(i).AAs]}];
    else
        tabledata(i,1:nonsamplecols)=[ {MutQual(i)} {MutQual_fracambig(i)} {annotations(i).type} {annotations(i).pos} ...
            {locustag} {annotations(i).gene} {annotations(i).annotation} {annotations(i).AApos} {[annotations(i).nts]} ...
            {[annotations(i).AAs]} ];
    end
...
oldtable=tabledata;
if QualSort==1
    [tabledata, positions] = sortrows(oldtable,2);
end
