...
def annotateSNP(tree, dict, chromosome, pos):
	...
	for n,i in enumerate(header):
		if (i.startswith('Z') ) and len(i)<200:
		#this line CANNOT contain characters that are included within samplenames but not the first letter (case sensitive). See line 322 also
			locations.append(n)
...
def nameswap(tree, dictionary):
	...
	#	print(line)
		...
		while i < len(line):
			#print(line[i])
			if line[i] not in ['Z']: #S for to_mark_strain_name... #See line 255 also
				newtree+=line[i]
				i+=1
			else:
				remainder=line[i:]
