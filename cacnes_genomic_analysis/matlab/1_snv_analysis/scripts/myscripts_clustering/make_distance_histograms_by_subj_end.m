...
%     end
    ...
    fs = 12;
    legend_labels = arrayfun(@(x) [ 'lineage ' num2str(x) ], next_subject_clusters, 'UniformOutput', false );
    %
    % same cluster
    subplot(5,1,1)
    hold on
    ...
    set(gca, 'FontSize', fs )
    title('distance to the closest colony in the lineage cluster')
    ...
    set(gca, 'FontSize', fs )
    title('distance to the closest colony in a different lineage (same subject)')
    ylabel('num of colonies')
    l=legend( legend_labels, 'Location', 'northeastoutside' );
    l.Title.String = [ 'subject ' next_subject_char ' clusters' ];
    l.Title.FontSize = fs;
    ...
    ylim( [ 0 1.1*max(sum(counts)) ] )
    set(gca, 'FontSize', fs )
    title('distance to closest colony in a different lineage (different subject)')
    ylabel('num of colonies')
    l=legend( legend_labels, 'Location', 'northeastoutside' );
    l.Title.String = [ 'subject ' next_subject_char ' clusters' ];
    l.Title.FontSize = fs;
    ...
    set(gca, 'FontSize', fs )
    title('distance to closest colony in any lineage (same subject)')
    ylabel('num of colonies')
    ...
    yticks( 0:1:max(1,1.1*max(counts)) )
    set(gca, 'FontSize', fs )
    title('distance to closest colony in any lineage (different subject)')
    ...
    print([ dir_save '/' 'cluster-hists-min_subj-' next_subject_char '.png' ],'-dpng')
...
yticks( 0:100:ymax_1 )
set(gca, 'FontSize', fs )
title('distance to closest colony in the same lineage')
yl=ylabel('# colonies');
...
yticks( 0:100:ymax_1 )
set(gca, 'FontSize', fs )
title('distance to closest colony in another lineage (same subject)')
yl=ylabel('# colonies');
set(yl, 'Units', 'Normalized', 'Position', [yl_pos, 0.5, 0]);
hold off
%
...
yticks( 0:100:ymax_1 )
set(gca, 'FontSize', fs )
title('distance to closest colony in another lineage (different subject)')
yl=ylabel('# colonies');
set(yl, 'Units', 'Normalized', 'Position', [yl_pos, 0.5, 0]);
hold off
%
...
set(gcf, 'PaperPositionMode', 'manual');
set(gcf, 'PaperPosition', [0 0 12 16]);
print([ dir_save '/' 'cluster-hists-min_subj-ALL.png' ],'-dpng')
...
print([ dir_save '/' 'cluster-hists-min_subj-ALL_legend.png' ],'-dpng')
