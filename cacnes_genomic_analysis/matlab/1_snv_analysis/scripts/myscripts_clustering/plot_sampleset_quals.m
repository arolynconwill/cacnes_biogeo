function plot_sampleset_quals( mySamplesetQuals, p_important, p_all, plotSampleNames, plotSampleOrder, mySamplesetName, cluster_filtering_directory_name )

% This function makes a heatmap of sample MAFs across interesting
% positions for a set of samples.

% Inputs: 
% mySamplesetQuals = major allele frequency (MAF) table
...
%% Version history:
% 2019.12.03 Arolyn: Quals plto based from MAFs plot
% ...
...
% Qualss at important positions only
mySamplesetQualsImportantPositions = transpose(mySamplesetQuals(p_important,:));
...
figure(1)
clf(1,'reset')
figCov=heatmap(xvalues, yvalues, mySamplesetQualsImportantPositions(plotSampleOrder,pos_min:pos_skip:pos_max));
% Color scheme
...
figCov.Title = [ 'Quals at Important Positions: ' mySamplesetName ];
...
print([pwd '/' cluster_filtering_directory_name '/' 'Figure_' mySamplesetName '_Quals.png'],'-dpng')
