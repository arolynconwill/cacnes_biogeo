...
%write input file
tempnames = generate_phylip_input(calls, newnames, [timestamp '_infile.txt']);
...
%if ~exist('outfile','file') % commented out 2021.03.30 because matlab is
%no longer returning the correct result; seems to be specific to filenames
%without extensions; no idea what changed...
    fid = fopen('outfile','w');
    fprintf(fid, 'foo');
    fclose(fid);
    fid = fopen('outtree','w');
