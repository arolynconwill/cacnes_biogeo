...
def build_sample_file_list(file,dir_data):
	''' read clade file with sample IDs and transform to files (based on known structure) '''
	# Arolyn update: additional input for data file directory (for referencing centralized proessed data)
	# with open(file, "r") as ins:
	ls_fwd = []
	ls_rev = []
	for line in file:
		#line = line.rstrip('\n')
		ls_fwd.append(dir_data+line+"_filt1.fq.gz")
		ls_rev.append(dir_data+line+"_filt2.fq.gz")
